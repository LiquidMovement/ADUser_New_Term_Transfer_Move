    ...
    param(
        ...
        function removeGroups{   #function that removes Group Memberships except Domain Users & FileShares##
            ...
            foreach($sGroup in $ADGroups){
                ...
                if($AcctName -notlike "*Domain Users*"){
                    "$AcctName" | out-file "\\<NETWORK PATH>\UsersRemovedGroupMemberships\$UPN-Transfer_RemovedGroups.txt" -Append
        ...
        function MoveOU{
            if($Dept -eq "Accounting"){
                $OU = "OU=Accounting,OU=XXXX,OU=XXXX,DC=corp,DC=YOURDOMAIN,DC=com"
                Move-ADObject -Identity $dName -TargetPath $OU
            }
            elseif($Dept -eq "Cashiers"){
                $OU = "OU=Cashiers,OU=XXXX,OU=XXXX,DC=corp,DC=YOURDOMAIN,DC=com"
                Move-ADObject -Identity $dName -TargetPath $OU
            }
            elseif($Dept -eq "Compliance"){
                $OU = "OU=Complaince,OU=XXXX,OU=XXXX,DC=corp,DC=YOURDOMAIN,DC=com"
                Move-ADObject -Identity $dName -TargetPath $OU
            }
            elseif($Dept -eq "CustomerService"){
                $OU = "OU=Customer Service,OU=XXXX,OU=XXXX,DC=corp,DC=YOURDOMAIN,DC=com"
                Move-ADObject -Identity $dName -TargetPath $OU
            }
            elseif($Dept -eq "DefaultDepts"){
                $OU = "OU=Default Depts,OU=XXXX,OU=XXXX,DC=corp,DC=YOURDOMAIN,DC=com"
                Move-ADObject -Identity $dName -TargetPath $OU
            }
            elseif($Dept -eq "ExecAdmin"){
                $OU = "OU=Exec Admin,OU=XXXX,OU=XXXX,DC=corp,DC=YOURDOMAIN,DC=com"
                Move-ADObject -Identity $dName -TargetPath $OU
            }
            elseif($Dept -eq "ExecTeam"){
                $OU = "OU=Exec Team,OU=XXXX,OU=XXXX,DC=corp,DC=YOURDOMAIN,DC=com"
                Move-ADObject -Identity $dName -TargetPath $OU
            }
            elseif($Dept -eq "Facilities"){
                $OU = "OU=Facilities,OU=XXXX,OU=XXXX,DC=corp,DC=YOURDOMAIN,DC=com"
                Move-ADObject -Identity $dName -TargetPath $OU
            }
            elseif($Dept -eq "HECM"){
                $OU = "OU=HECM,OU=XXXX,OU=XXXX,DC=corp,DC=YOURDOMAIN,DC=com"
                Move-ADObject -Identity $dName -TargetPath $OU
            }
            elseif($Dept -eq "HR"){
                $OU = "OU=HR,OU=XXXX,OU=XXXX,DC=corp,DC=YOURDOMAIN,DC=com"
                Move-ADObject -Identity $dName -TargetPath $OU
            }
            elseif($Dept -eq "Insurance"){
                $OU = "OU=Insurance,OU=XXXX,OU=XXXX,DC=corp,DC=YOURDOMAIN,DC=com"
                Move-ADObject -Identity $dName -TargetPath $OU
            }
            elseif($Dept -eq "InvestorReporting"){
                $OU = "OU=Investor Reporting,OU=XXXX,OU=XXXX,DC=corp,DC=YOURDOMAIN,DC=com"
                Move-ADObject -Identity $dName -TargetPath $OU
            }
            elseif($Dept -eq "Origination"){
                $OU = "OU=Origination,OU=XXXX,OU=XXXX,DC=corp,DC=YOURDOMAIN,DC=com"
                Move-ADObject -Identity $dName -TargetPath $OU
            }
            elseif($Dept -eq "Shipping"){
                $OU = "OU=Shipping,OU=XXXX,OU=XXXX,DC=corp,DC=YOURDOMAIN,DC=com"
                Move-ADObject -Identity $dName -TargetPath $OU
            }
            elseif($Dept -eq "Supervisors"){
                $OU = "OU=Supervisors,OU=XXXX,OU=XXXX,DC=corp,DC=YOURDOMAIN,DC=com"
                Move-ADObject -Identity $dName -TargetPath $OU
            }
            elseif($Dept -eq "TaxesPayoffs"){
                $OU = "OU=Taxes_Payoffs,OU=XXXX,OU=XXXX,DC=corp,DC=YOURDOMAIN,DC=com"
                Move-ADObject -Identity $dName -TargetPath $OU
            }
            elseif($Dept -eq "Treasury"){
                $OU = "OU=Treasury,OU=XXXX,OU=XXXX,DC=corp,DC=YOURDOMAIN,DC=com"
                Move-ADObject -Identity $dName -TargetPath $OU
            }
            elseif($Dept -eq "IT"){
                $OU = "OU=XXXX,OU=XXXX,DC=corp,DC=YOURDOMAIN,DC=com"
                Move-ADObject -Identity $dName -TargetPath $OU
            }
            else{
        ...
        function restoreGroups{       #function that removes Group Memberships except Domain Users
            $prevMember = Get-Content -path "\\<Network Path>\UsersRemovedGroupMemberships\$UPN-Transfer_RemovedGroups.txt"     #retrieves the text file created during the users Disabling via the Disable Script
        ...
        While($test -eq $false){    #INNER WHILE LOOP
            ...
            if($x -eq "Y"){
                $test = $true
                Write-host "`n$UPN will have`n1.Group memberships (except Domain Users) removed. `n2. AD info from $UserCopyFrom copied to $script:CopyToUser `n& A text file containing all removed Group Memberships will be saved to 'K:\UsersRemovedGroupMemberships'."
            }
            else{
                $test = $false
